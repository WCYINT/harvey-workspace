...
SMTP_PASS = os.environ["HARVEY_EMAIL_AUTH"]  # James's authorization code (env var only)

def get_revision_loop_status() -> dict:
    """Verify self_revision_loop.py and its LaunchAgent."""
    status = {"script_exists": False, "launchagent_running": False, "last_run": "从未"}
    script_path = WORKSPACE / ".scripts" / "self_revision_loop.py"
    if script_path.exists():
        status["script_exists"] = True
    try:
        r = subprocess.run(
            ["launchctl", "list", "com.hjtech.self-revision-loop"],
            capture_output=True, text=True, timeout=5
        )
        status["launchagent_running"] = r.returncode == 0 and "com.hjtech.self-revision-loop" in r.stdout
        # Try to get last run time from stdout
        for line in r.stdout.splitlines():
            if "LastExitStatus" in line or "PID" in line:
                status["last_run"] = line.strip()[:40]
                break
    except Exception:
        pass
    return status
...
def get_system_stats() -> dict:
    ...
    return {
        ...
        "al_stats": al_stats,
        "revision_status": get_revision_loop_status(),
    }
...
<table style="width: 100%; border-collapse: collapse; margin-bottom: 20px;">
    ...
    <tr>
        ...
        <td style="padding: 12px; border: 1px solid #e2e8f0; color: {'green' if stats.get('revision_status',{}).get('script_exists') else 'red'};">{'✅ 存在' if stats.get('revision_status',{}).get('script_exists') else '❌ 缺失'}</td>
        <td style="padding: 12px; border: 1px solid #e2e8f0;">LA:{stats.get('revision_status',{}).get('launchagent_running','?')}| {stats.get('revision_status',{}).get('last_run','?')}</td>
    </tr>
</table>

<h2 style="color: #333; border-bottom: 2px solid #667eea; padding-bottom: 8px;">📊 auto_learner 实时状态</h2>

{stats.get('al_stats', {}).get('raw', '<p>无法获取统计信息</p>') if isinstance(stats.get('al_stats'), dict) else '<p>无法获取统计信息</p>'}
...
<h2 style="color: #333; border-bottom: 2px solid #667eea; padding-bottom: 8px;">⚠️ 待处理事项（仅供参考）</h2>

<div style="background: #fff7ed; border-radius: 8px; padding: 16px; border-left: 4px solid #f97316; margin-bottom: 20px;">
    <strong style="color: #c2410c;">GitHub Push secret scanning</strong>
    <p style="margin: 8px 0 0; color: #555;">twilio secret in commit ba8ce17 被 GH secret scanning 拦截</p>
    <p style="margin: 4px 0 0; color: #555;">如未处理：访问 <a href="https://github.com/WCYINT/harvey-workspace/security/secret-scanning/unblock-secret/3BT01IqN2jOwsPLwSHUFLsi3AUs">secret scanning unblock link</a> 点击 "Allow secret"</p>
</div>

<div style="background: #dcfce7; border-radius: 8px; padding: 16px; border-left: 4px solid #16a34a;">
    <strong style="color: #166534;">✅ 163邮箱 SMTP 已恢复</strong>
    <p style="margin: 8px 0 0; color: #555;">授权码已于 2026-03-27 更新为环境变量 HARVEY_EMAIL_AUTH，无需 James 干预</p>
</div>
