...
def send_feishu_message(token, content, receive_id, receive_id_type="open_id"):
...
def format_report(usage_data):
    ...
    interval_used = m27_data["current_interval_usage_count"]
    interval_total = m27_data["current_interval_total_count"]
    interval_pct = interval_used / interval_total * 100
...
{status(interval_pct)} 已用约 {interval_used:,} / {interval_total:,} ({interval_pct:.1f}%)
