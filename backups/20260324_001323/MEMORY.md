...
- **session_status 正确解读（铁律）**：
  ...
  - `13%` = 已消耗的 API 调用次数占比
  - `⏱2h 18m` = 该时间段剩余时间
  - **95% 阈值定义**：指的是 `5h XX% left` 中的 `XX%`，即5小时窗口内已消耗的 API 调用次数比例
- **检查频率**：每 25 分钟通过 cron job 检查一次 usage
- **usage < 95%**：正常调用 MiniMax API，保持正常工作
- **usage >= 95%**：减少 API 消耗，自主决策并执行，邮件通知 wcyint@163.com
- **减少消耗决策**：
  - >= 99%：切换 kimi-k2.5（完全停止 MiniMax）、关闭非必要任务
  - >= 97%：切换 kimi-k2.5、降低 thinking 级别、合并批量操作
  - >= 95%：降低请求频率、使用缓存、减少调试输出
