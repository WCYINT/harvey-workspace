...
This skill collects personal data (name, email, address, job details) and sends it over HTTPS to the Blossom API. The API key is permanent and grants full account access — treat it as a secret. No data is stored locally.
...
2. **Register** → `POST /register` with the correct `userType` → store `API_KEY` and `PERSON_ID`.
...
4. **Talk** → `POST /ask` with whatever the user says. The server handles everything based on the account type: job matches, candidate lists, applications, screening questions, status updates.
...
Send the user's message as-is. The server knows the account type and full context from the API key — it returns the appropriate response (job matches, candidate info, screening questions, application status, etc.). Just relay the result.
